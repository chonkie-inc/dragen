"""dragen - CodeAct-style AI agent framework."""

from .dragen import Agent, Context, ToolInfo
from .dragen import Sandbox as _RustSandbox
import inspect


class Sandbox:
    """A secure Python sandbox with ``@sandbox.tool`` decorator support.

    Wraps the Rust sandbox and adds Python-level conveniences::

        sandbox = Sandbox(builtins=True)

        @sandbox.tool
        def search(query: str, limit: int = 5) -> list:
            '''Search the web.'''
            return [{"result": query}]

        sandbox["document"] = "..."
        result = sandbox("search('hello')")
    """

    def __init__(self, builtins=True):
        self._inner = _RustSandbox(builtins=builtins)
        self._tools: dict[str, dict] = {}

    def __getattr__(self, name):
        # Delegate run, capture, set, limit, register, mount, files, module
        # to the Rust sandbox.
        return getattr(self._inner, name)

    def __call__(self, code: str):
        """Allow sandbox(code) as shorthand for sandbox.run(code)."""
        return self._inner.run(code)

    def __setitem__(self, name: str, value):
        """Allow sandbox["x"] = val as shorthand for sandbox.set("x", val)."""
        self._inner.set(name, value)

    def tool(self, func=None, *, name=None):
        """Register a function as a sandbox tool.

        Usage::

            @sandbox.tool
            def add(a: int, b: int) -> int:
                '''Add two numbers.'''
                return a + b

            @sandbox.tool(name="search")
            def web_search(query: str) -> list:
                '''Search the web.'''
                return []
        """

        def _register(fn):
            tool_name = name or fn.__name__
            sig = inspect.signature(fn)
            params = list(sig.parameters.values())

            def wrapper(args):
                kwargs = {}
                for i, p in enumerate(params):
                    if i < len(args):
                        kwargs[p.name] = args[i]
                    elif p.default is not inspect.Parameter.empty:
                        kwargs[p.name] = p.default
                    else:
                        raise TypeError(
                            f"{tool_name}() missing required argument: '{p.name}'"
                        )
                return fn(**kwargs)

            self._inner.register(tool_name, wrapper)
            self._tools[tool_name] = {
                "sig": sig,
                "doc": inspect.getdoc(fn),
            }
            return fn

        if func is not None:
            return _register(func)
        return _register

    def describe(self) -> str:
        """Generate Python-style tool docs for LLM system prompts.

        Combines tools registered via Rust ``register()`` and
        Python ``@sandbox.tool``.
        """
        parts = []

        rust_desc = self._inner.describe()
        if rust_desc.strip():
            parts.append(rust_desc)

        for tool_name, meta in self._tools.items():
            sig = meta["sig"]
            doc = meta["doc"] or ""

            param_strs = []
            for p in sig.parameters.values():
                s = p.name
                if p.annotation is not inspect.Parameter.empty:
                    s += f": {getattr(p.annotation, '__name__', str(p.annotation))}"
                if p.default is not inspect.Parameter.empty:
                    s += f" = {p.default!r}"
                param_strs.append(s)

            ret = ""
            if sig.return_annotation is not inspect.Signature.empty:
                ret_name = getattr(
                    sig.return_annotation, "__name__", str(sig.return_annotation)
                )
                ret = f" -> {ret_name}"

            parts.append(
                f"def {tool_name}({', '.join(param_strs)}){ret}:\n"
                f'    """{doc}"""'
            )

        return "\n\n".join(parts)


__all__ = ["Agent", "Context", "Sandbox", "ToolInfo"]
